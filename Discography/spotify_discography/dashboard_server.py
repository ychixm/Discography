...
dashboard_server.py
===================
Serveur HTTP léger pour le dashboard de monitoring.
...
Routes GET :
  ...
  /api/db/artist-detail         → détail d'un artiste (id=...)
  /api/db/album-checkpoints     → checkpoints album actifs (reprises en cours)
  /api/db/rate-limit-intervals  → historique des intervalles 429 (limit=N)
  /api/db/rate-limit-stats      → agrégats rate limit

Routes POST :
  /api/blacklist-track          → blackliste une track retirée
  /api/setup/create-config      → crée config.json au premier démarrage
"""
import json
import sqlite3
import threading
...
def update_run_state(**kwargs):
    with _run_lock:
        for k, v in kwargs.items():
            ...
            elif k == "daemon_meta" and isinstance(v, dict):
                _run_state["daemon_meta"].update(v)
            else:
                _run_state[k] = v
...
def _album_checkpoints_list() -> list:
    ...
    try:
        conn = _db_connect()
        rows = conn.execute("""
            SELECT ac.artist_id, ac.album_ids, ac.last_album_idx,
                   ar.artist_name
            FROM album_checkpoint ac
            LEFT JOIN artists ar ON ar.artist_id = ac.artist_id
        ...
        for r in rows:
            ...
            result.append({
                ...
                "total_albums":      total,
                "albums_done":       done,
                "albums_remaining":  total - done,
                "last_album_idx":    r["last_album_idx"],
            })
        return result
    except Exception:
        return []
    ...
    def do_GET(self):
        ...
        if path == "/api/run":
            with _run_lock:
                ...
                data["daemon_meta"]  = dict(data["daemon_meta"])
                data["artists_run"]  = list(data["artists_run"])
        ...
        elif path == "/api/db/album-checkpoints":
            self._send_json(_album_checkpoints_list())
    ...
    def do_POST(self):
        ...
        if path == "/api/blacklist-track":
            ...
            if not artist_id or not track_id:
                self._send_json(
                    {"error": "artist_id et track_id requis"}, 400
                )
            else:
...
def start(port: int | None = None) -> int:
    ...
    for p in candidates:
        try:
            ...
            logger.warning(
                "Port %d indisponible (%s), essai suivant…", p, e.strerror
            )

    logger.error(
        "Dashboard désactivé : aucun port disponible parmi %s", candidates
    )
    return 0
